		...
		while(true) {
			...
			if(a.length()-i<0&&b.length()-i<0) {
				...
				if(x+y>=10) {           //이전 싸이클에서 계산값이 10이 넘었을 때 처리.
					sb.append(b.charAt(b.length()-i)-48+1);//b에 숫자만 남았을때 그대로 b의 수를 넣어줌.  (48은 charAt를 해서 받아온 숫자는 아스키코드 값이기에 -48을 해줌)
				}else {
				...
				if(x+y>=10) {
					sb.append(a.charAt(a.length()-i)-48+1);
				}else {
				...
				y =b.charAt(b.length()-i)-48;                 //이전 싸이클에서 계산한 x+y가 10보다 클 경우 현재 x+y의 값에 1을 더해줌과 동시에 10으로 나눈 나머지 값을 StringBuilder에 추가해줌.
				sb.append((x+y+1)%10);
				
			}else {
			
				x =a.charAt(a.length()-i)-48;				//이전 싸이클에 x+y가 10이하인 경우 아무처리없이 x,y를 새로 정의하고 더한 값을 StringBuilder에 넣어줌.
